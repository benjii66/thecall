...
export function MatchList({ matches }: { matches: MatchListItem[] }) {
  ...
  hoverEffect
  className="
    w-full cursor-pointer p-4 text-left group
    shadow-[0_0_0_1px_rgba(255,255,255,0.03),0_18px_60px_rgba(0,0,0,0.55)]
  "
>
  {/* Mini glow interne (plus subtil que la landing pour pas charger la liste) */}
  <div className="pointer-events-none absolute -top-10 -left-10 h-40 w-40 rounded-full bg-cyan-500/10 blur-[45px] opacity-35 mix-blend-screen transition-opacity duration-500 group-hover:opacity-55" />

  {/* Sheen léger au hover */}
  <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100 bg-gradient-to-r from-cyan-500/0 via-cyan-500/6 to-cyan-500/0" />
  ...
  <div className="flex items-center gap-3 sm:gap-4">
    ...
    {/* VS */}
    <div className="hidden sm:block text-white/30 text-sm">
      {t("matchList.vs")}
    </div>
    ...
    >
      <span className="hidden sm:inline">
        {match.win ? t("matchList.victory") : t("matchList.defeat")}
      </span>
      <span className="sm:hidden">
        {match.win ? t("matchList.victoryShort") : t("matchList.defeatShort")}
      </span>
    </div>
  </div>
</GlassCard>
